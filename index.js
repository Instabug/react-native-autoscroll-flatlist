...
export default class AutoScrollFlatList extends React.PureComponent {
  constructor(props) {
    ...
    this.onScroll = event => {
      ...
      this.scrollTop = this.props.horizontal
    ...
    this.renderDefaultNewMessageAlertComponent = (
      newMessageCount,
      translateY
    ...
    this.renderDefaultIndicatorComponent = () => (
      <View
        style={
          this.props.indicatorContainerStyle ?? styles.scrollToEndIndicator
        }
      >
        <Triangle />
      </View>
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      const newCount =
        prevState.newMessageCount + data.length - prevProps.data.length;
        ...
        {showNewMessageAlert &&
          ...
          newMessageCount > 0 && (
            <TouchableWithoutFeedback onPress={() => this.scrollToEnd()}>
              {newMessageAlertRenderer
                ? newMessageAlertRenderer(newMessageCount, messageAlertY)
                : this.renderDefaultNewMessageAlertComponent(
                    newMessageCount,
                    messageAlertY
                  )}
